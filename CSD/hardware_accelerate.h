...
#define F2FS_SLOT_LEN		8       // 涓�涓洰褰曟潯鐩Ы鍖呭惈8瀛楄妭闀跨殑鏂囦欢鍚�
...
#define DEV_ADDR 0x10000000+16*8*4*16384 //defined the gc buffer,this place is not important.
//**************for file system*****************
// void _setBitmap(unsigned int location);
int _getBitmap(unsigned int location);
void updateSB(unsigned int dataAddr);
void updateCP(unsigned int dataAddr);
void initialMetadata(unsigned int startLBA);
...
__le32 read_NAT(int nid,struct f2fs_super_block  *sb,struct f2fs_checkpoint *ckpt);
...
/*resolution file path, filename saves the path user delivers,dir saves next directory and subset saves and return the length of dir*/
int extract_dir(const char* path, const unsigned int path_len, unsigned int *dir_offset, unsigned int *dir_len);
/*璁＄畻dentry block涓璪itmap淇濆瓨鐨勬湁鏁坉entry鐨勬暟閲�*/
...
__le32 f2fs_path(char* filename, unsigned int len);
