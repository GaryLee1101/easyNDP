...
int CheckSearchTaskConfigDMA(){
	if(check_auto_rx_dma_partial_done(searchTask->rxDmaTail, searchTask->rxDmaOverFlowCnt)){
		searchTask->rxDmaExe = 0;

		char* index = (char*)DMA_TASK_CONFIG_ADDR;  // copy addr
		strcpy(searchTask->targetString, index);

		if (searchTask->need_path_walk) {  // need path walk
			index += 16;  // skip the target string
			unsigned int path_len = *((unsigned int *)index);
			index += 4;
			char path[path_len + 1];
			memcpy(path, index, path_len);
			path[path_len] = '\0';

			XTime_GetTime(&time_start_retrieve);
			unsigned int file_ino = f2fs_path(path, path_len);
			
			read_inode(file_ino);
			
			XTime_GetTime(&time_end_retrieve);
			// XTime_GetTime(&time_end_search);
			// set_auto_nvme_cpl(searchTask->cmdSlotTag, 0, 0);
			...
			// tUsed = t_read_block * 1000000 / COUNTS_PER_SECOND;
			// printf("t_read_block: %d us.\r\n", tUsed);
			// tUsed = t_memory_copy * 1000000 / COUNTS_PER_SECOND;
			// printf("t_memory_copy: %d us.\r\n", tUsed);
			// tUsed = t_read_nat * 1000000 / COUNTS_PER_SECOND;
			// printf("t_read_nat: %d us.\r\n", tUsed);
			// tUsed = t_path_hash * 1000000 / COUNTS_PER_SECOND;
			// printf("t_path_hash: %d us.\r\n", tUsed);
			// tUsed = t_count_dentry * 1000000 / COUNTS_PER_SECOND;
			// printf("t_count_dentry: %d us.\r\n", tUsed);
			// tUsed = t_find_dentry * 1000000 / COUNTS_PER_SECOND;
			// printf("t_find_dentry: %d us.\r\n", tUsed);
			...
			unsigned int blk_addr, blk_num;
			retrieve_address(file_ino, &blk_addr, &blk_num);
			
			analysisTask(blk_addr, blk_num);
		}
		else {
			index += 20;  // 16*sizeof(char) + sizeof(int)
			struct addressBlock* content = (struct addressBlock*)index;
			while(1){
				analysisTask(content->blockAddr, content->blockNum);
				if(content->endFlag){
					content ++;
					break;
				}
...
void PushToReqQueue(P_LOW_LEVEL_REQ_INFO lowLevelCmd)
{
	...
	if(lowLevelCmd->request >= LLSCommand_RxDMA)
	{
		...
		reqQueue->reqEntry[rear][chNo][wayNo].task_index = 0;
	...
	else
	{
		...
		reqQueue->reqEntry[rear][chNo][wayNo].task_index = 0;
		if(lowLevelCmd->search){  // for searching
			reqQueue->reqEntry[rear][chNo][wayNo].pageDataBuf = SEARCH_PAGE_DATA_BUFFER_ADDR + (wayNo * 8 + chNo) * PAGE_SIZE;
		}
		else if(lowLevelCmd->task_index){  //belong to a task
			reqQueue->reqEntry[rear][chNo][wayNo].task_index = lowLevelCmd->task_index;
			reqQueue->reqEntry[rear][chNo][wayNo].offset_in_task = lowLevelCmd->offset_in_task;
			reqQueue->reqEntry[rear][chNo][wayNo].sector_start = lowLevelCmd->sector_start;
			reqQueue->reqEntry[rear][chNo][wayNo].sector_count = lowLevelCmd->sector_count;
			reqQueue->reqEntry[rear][chNo][wayNo].pageDataBuf = (unsigned int)task_buffer_list[lowLevelCmd->task_index-1].buffer;
		}
		...
		reqQueue->reqEntry[rear][chNo][wayNo].search = lowLevelCmd->search;
		// reqQueue->reqEntry[rear][chNo][wayNo].searchBufferEntry = lowLevelCmd->searchBufferEntry;
		reqQueue->reqEntry[rear][chNo][wayNo].searchPageIndex = lowLevelCmd->searchPageIndex;
		rqPointer->rqPointerEntry[chNo][wayNo].rear = (rear + 1) % REQ_QUEUE_DEPTH;
	}
}
...
int PopFromReqQueue(int chNo, int wayNo)
{
	...
	if (request == LLSCommand_RxDMA)
	{
		...
		unsigned int pageDataAddr = BUFFER_ADDR + bufferEntry * BUF_ENTRY_SIZE;
		unsigned int lpn = bufMap->bufEntry[bufferEntry].lpn;
		// monitor address region
		if (lpn * 4 == FS_OFFSET){  // hit f2fs_super_block
			updateSB(pageDataAddr);
		}
		else if (lpn * 4 == (FS_OFFSET + 512) || lpn * 4 == (FS_OFFSET + 1024)){
			// hit cp
			XTime start, end;
			XTime_GetTime(&start);
			updateCP(pageDataAddr);
			XTime_GetTime(&end);

			unsigned int tUsed;
			tUsed = (end - start) * 1000000 / COUNTS_PER_SECOND;
			printf("time of updateCP: %d us.\r\n", tUsed);
		}
	...
	switch(dieStatusTable->dieStatusEntry[chNo][wayNo].dieStatus)
	{
		...
		case DS_EXE:
			if(reqStatus == RS_DONE)
			{
				...
				else if(reqQueue->reqEntry[front][chNo][wayNo].request == V2FCommand_ReadPageTransfer && reqQueue->reqEntry[front][chNo][wayNo].task_index)
				{
					unsigned int task_index = reqQueue->reqEntry[front][chNo][wayNo].task_index - 1;
					unsigned int offset_in_task = reqQueue->reqEntry[front][chNo][wayNo].offset_in_task;
					unsigned int sector_start = reqQueue->reqEntry[front][chNo][wayNo].sector_start;
					unsigned int sector_count = reqQueue->reqEntry[front][chNo][wayNo].sector_count;

					if(DEBUGMODE){
						xil_printf("read data done.\r\n");
						xil_printf("task index:%u offset:%u start:%u count:%u\r\n",task_index, offset_in_task, sector_start, sector_count);
					}
					handle_task(task_index, offset_in_task, sector_start, sector_count, reqQueue->reqEntry[front][chNo][wayNo].pageDataBuf);
			...
			{
				if(retryLimitTable->retryLimitEntry[chNo][wayNo] > 0)  //濡傛灉杩樻湁retry鏈轰細,retry
				{
					retryLimitTable->retryLimitEntry[chNo][wayNo]--;  //retry鏈轰細--
...
/*
	渚濇嵁姝ay firstQueue headEntry鐨剅equest绫诲瀷瀵规way閲嶆柊鏍囪;
	鑻irstQueue绌�,鍒欏弬鐓econdQueue;
	鑻econdQueue涔熺┖,鍒欏皢姝ay鏍囪涓篿dle
*/
void FindPriorityTable(int chNo, int wayNo, int firstQueue)
{
	...
	{
		empty = srqPointer->rqPointerEntry[chNo][wayNo].front ==  srqPointer->rqPointerEntry[chNo][wayNo].rear;
		if(empty)  //SubReqQueue empty
		{
			dieStatusTable->dieStatusEntry[chNo][wayNo].subReqQueueEmpty = 1;
			dieStatusTable->dieStatusEntry[chNo][wayNo].queueSelect = REQ_QUEUE;  //switch to ReqQueue
			empty = rqPointer->rqPointerEntry[chNo][wayNo].front ==  rqPointer->rqPointerEntry[chNo][wayNo].rear;
			if(empty)  //ReqQueue empty
			{
				dieStatusTable->dieStatusEntry[chNo][wayNo].reqQueueEmpty = 1;
				LinkToIdle(chNo, wayNo);  //mark this way IDLE
				return;
			}
			request = reqQueue->reqEntry[rqPointer->rqPointerEntry[chNo][wayNo].front][chNo][wayNo].request;  //ReqQueue not empty,take the front entry's request
		}
		else  //SubReqQueue not empty
		{
			dieStatusTable->dieStatusEntry[chNo][wayNo].queueSelect = SUB_REQ_QUEUE;  //switch to SubReqQueue
			request = subReqQueue->reqEntry[srqPointer->rqPointerEntry[chNo][wayNo].front][chNo][wayNo].request;  //take the front entry's request
		}
	}
...
int ExeLowLevelReqPerCh(int chNo, int firstQueue)  //鎵ц姝hannel鐨勯槦鍒楄皟搴�
{
	int wayNo, idleWay, nextWay, enable, reqStatus, statusOption;
	unsigned int readyBusy;

	if(wayPriorityTable->wayPriorityEntry[chNo].idleHead != 0xf)  //姝hannel瀛樺湪idleWay锛岄亶鍘嗘瘡涓猧dleWay浠ョ‘瀹氭槸鍚︿粛idle
	{
		wayNo = wayPriorityTable->wayPriorityEntry[chNo].idleHead;  //鍙杊ead idleWay
		idleWay = 0;

		while(wayNo != 0xf)  //寮�濮嬮亶鍘嗘鏌�
		{
			...
			if(enable)  //Req not empty or SubReq not empty, 姝ay涓嶅啀idle
			{
				//浠庨摼琛ㄤ腑鍒犻櫎姝ay
				if((dieStatusTable->dieStatusEntry[chNo][wayNo].nextWay != 0xf) && (dieStatusTable->dieStatusEntry[chNo][wayNo].prevWay != 0xf))  //涓棿
				{
				...
				else if((dieStatusTable->dieStatusEntry[chNo][wayNo].nextWay == 0xf) && (dieStatusTable->dieStatusEntry[chNo][wayNo].prevWay != 0xf))  //鏈�鍙�
				{
				...
				else if((dieStatusTable->dieStatusEntry[chNo][wayNo].nextWay != 0xf) && (dieStatusTable->dieStatusEntry[chNo][wayNo].prevWay == 0xf))  //鏈�宸�
				{
				...
				nextWay = dieStatusTable->dieStatusEntry[chNo][wayNo].nextWay;  //鍙栦笅涓�涓獁ay鏆傚瓨
				FindPriorityTable(chNo, wayNo, firstQueue);
				wayNo = nextWay;
			}
			else  //姝ay鐨勪袱涓槦鍒楅兘绌�,浠峣dle
			{
				idleWay++;
		...
		if(idleWay == WAY_NUM){
			if(searchTask->taskValid && searchTask->rxDmaExe)
				CheckSearchTaskConfigDMA();
			return 0;
		}
	}
	if(wayPriorityTable->wayPriorityEntry[chNo].statusReportHead != 0xf)  //姝hannel瀛樺湪statusReport way, 閬嶅巻浠ョ‘瀹歴tatus
	{
		readyBusy = V2FReadyBusyAsync(chCtlReg[chNo]);
		wayNo = wayPriorityTable->wayPriorityEntry[chNo].statusReportHead;  //鍙杊ead

		while(wayNo != 0xf)  //寮�濮嬮亶鍘�
		{
			if ((readyBusy >> wayNo) & 1)
			{
				if(dieStatusTable->dieStatusEntry[chNo][wayNo].queueSelect == REQ_QUEUE)
				...
				if(reqStatus != RS_RUNNING)  //鑻ラ槦鍒楃姸鎬佷笉鏄疪unning
				{
					if((dieStatusTable->dieStatusEntry[chNo][wayNo].nextWay != 0xf) && (dieStatusTable->dieStatusEntry[chNo][wayNo].prevWay != 0xf))  //涓棿
					{
						dieStatusTable->dieStatusEntry[chNo][dieStatusTable->dieStatusEntry[chNo][wayNo].prevWay].nextWay = dieStatusTable->dieStatusEntry[chNo][wayNo].nextWay;
					...
					ExeLowLevelReqPerDie(chNo, wayNo, reqStatus);  //杩涘叆鐘舵�佹満璋冨害
	...
	if(wayPriorityTable->wayPriorityEntry[chNo].nvmeDmaHead != 0xf)  //姝hannel瀛樺湪闇�瑕佽繘琛宯vmeDma鎿嶄綔鐨剋ay,妫�鏌ユ搷浣滄槸鍚﹀畬鎴�
	{
		wayNo = wayPriorityTable->wayPriorityEntry[chNo].nvmeDmaHead;  //鍙杊ead
		...
		while(wayNo != 0xf)  //寮�濮嬮亶鍘�
		{
			...
			if(enable)
			{
				...
				nextWay = dieStatusTable->dieStatusEntry[chNo][wayNo].nextWay;
				FindPriorityTable(chNo, wayNo, firstQueue);  //閲嶆柊纭畾姝ay灞炰簬鍝釜閾捐〃
				wayNo = nextWay;
			}
			else
				wayNo = dieStatusTable->dieStatusEntry[chNo][wayNo].nextWay;
