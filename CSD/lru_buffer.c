...
unsigned int AllocateBufEntry(unsigned int lpn)
{
	...
	if(bufMap->bufEntry[evictionEntry].dirty)
	{
		...
		// xil_printf("lpn %d dirty, write back...\r\n", lpn);
		PmWrite(&bufferCmd);
...
void LRUBufRead(P_HOST_REQ_INFO hostCmd)
{
	...
	unsigned int loop = ((hostCmd->curSect % SECTOR_NUM_PER_PAGE) + hostCmd->reqSect) / SECTOR_NUM_PER_PAGE;  //((curSect % 4) + reqSec) / 4

	counter = 0;
	dmaIndex = 0; //dma offset
	tempLpn = hostCmd->curSect / SECTOR_NUM_PER_PAGE;

	hitEntry = CheckBufHit(tempLpn);
	if(hitEntry != 0x7fff)
	{
		devAddr = BUFFER_ADDR + hitEntry * BUF_ENTRY_SIZE + (hostCmd->curSect % SECTOR_NUM_PER_PAGE) * SECTOR_SIZE_FTL; //DRAM address of first sector
		if(loop)
			subReqSect = SECTOR_NUM_PER_PAGE - (hostCmd->curSect % SECTOR_NUM_PER_PAGE);  //4 - (curSect % 4)
		else
